MimiGames.register({
  id: "dune-buggy-rally",
  title: "Dune Buggy Rally 3D",
  emoji: "🏜️",
  category: "Sports",
  tags: ["3D"],
  players: "1P",
  howTo: "A pseudo-3D desert time trial. Left/Right or A/D to steer, Down/S to brake, no input to floor it. Drive between each pair of checkpoint flags — clean checkpoints shave time off your run, missed ones add a penalty. Clip a cactus or rock and you'll stall out (a time penalty and a moment to recover, but the run keeps going). Hit a dune ramp for big air — the longer you hang, the bigger the time bonus when you land. Reach the finish banner as fast as you can. Gamepad: D-pad left/right to steer, D-pad down to brake (Start un-hides the controller cursor if you need it). Touch: drag left/right to steer, hold BRAKE to slow down.",
  init(root, ctx) {
    ...
    const TRACK_HALF_WIDTH = 260;
    const COURSE_LENGTH = 3600;

    const SPEED_MAX = 400;
    const SPEED_MIN = 60;
    const GRAVITY_ACCEL = 90;
    const BRAKE_DECEL = 250;
    const TURN_DRAG = 45;
    const TURN_ACCEL = 350;
    const TURN_MAX_VX = 200;
    const TURN_DRAG_VX = 5.5;

    const COLLISION_RANGE = 24;
    const GATE_HALF_GAP = 48;
    const GATE_CHECK_WINDOW = 22;
    const CRASH_PENALTY = 1.5;
    ...
    const SETTINGS_KEY = "duneBuggySettings";
    const DIFFICULTIES = {
      easy: { label: "Easy", label2: "Groomed Dunes", obstacleDensity: 0.7, gateGap: GATE_HALF_GAP * 1.25 },
      normal: { label: "Normal", label2: "Rally Course", obstacleDensity: 1, gateGap: GATE_HALF_GAP },
      hard: { label: "Hard", label2: "Dakar Stage", obstacleDensity: 1.4, gateGap: GATE_HALF_GAP * 0.78 },
    };
    ...
    canvas.style.background = "#e8c07a";
    ...
    hint.textContent = "Left/Right = steer, Down = brake. Thread the checkpoints, dodge the hazards, catch the dunes.";
    ...
    function buildCourse() {
      ...
      gates = [];
      ramps = [];
      const hazardCount = Math.round(65 * diff.obstacleDensity);
      for (let i = 0; i < hazardCount; i++) {
        const z = 260 + Math.random() * (COURSE_LENGTH - 400);
        const side = Math.random() < 0.5 ? -1 : 1;
        const x = side * (60 + Math.random() * (TRACK_HALF_WIDTH - 40));
        hazards.push({ z, x, kind: Math.random() < 0.55 ? "🌵" : "🪨", hit: false });
      }
      let gz = 320;
      let swing = 1;
    ...
    function stall() {
      ...
      shakeTimer = 0.4;
      playSound("hit");
      ctx.setStatus(`Stalled out! +${CRASH_PENALTY.toFixed(1)}s penalty.`);
    }

    function updateGatesAndRamps() {
      gates.forEach((gt) => {
        ...
        if (rel < -GATE_CHECK_WINDOW) {
          ...
          if (clean) {
            ...
            spawnPopup(gt.z, gt.centerX, `-${CLEAN_GATE_BONUS.toFixed(1)}s Checkpoint!`, "#53e0ff");
            playSound("tick");
          } else {
            missedGates += 1;
    ...
    function updateCollisions() {
      if (player.airTime > 0 || player.invuln > 0 || devInvincible) return;
      hazards.forEach((t) => {
        if (t.hit) return;
    ...
    function finishRun() {
      ...
      playSound(stalls === 0 ? "success" : "lose");
      ...
      setTimeout(() => {
        ctx.showOverlay({
          title: "Finish!",
          subtitle: `Time: ${finalTime.toFixed(2)}s · Checkpoints: ${cleanGates}/${gates.length} · Stalls: ${stalls}${isNewBest ? " · New Best!" : ` · Best: ${Math.min(finalTime, best).toFixed(2)}s`}`,
          buttonText: "Rally Again",
          onButton: startGame,
        });
      }, 400);
    }
    ...
    function update(dt) {
      ...
      if (!over) {
        if (player.airTime > 0) {
          ...
          if (player.airTime <= 0) spawnPopup(player.worldPos, player.x, "Landed!", "#9bff8f");
        } else {
          if (heldBrake()) player.speed -= BRAKE_DECEL * dt;
          else player.speed += GRAVITY_ACCEL * dt;
          let turning = false;
        ...
        player.x = clamp(player.x, -TRACK_HALF_WIDTH + 20, TRACK_HALF_WIDTH - 20);
    ...
    function drawSky() {
      ...
      grad.addColorStop(1, "#f4d99a");
      g.fillStyle = grad;
      g.fillRect(0, 0, SCREEN_W, HORIZON_Y);
      g.fillStyle = "rgba(255,244,200,0.9)";
      g.beginPath();
      g.arc(SCREEN_W * 0.25, HORIZON_Y * 0.4, 34, 0, Math.PI * 2);
      g.fill();
      g.fillStyle = "rgba(200,150,90,0.55)";
      const parallax = (player.worldPos * 0.02) % 160;
      for (let i = -1; i < 8; i++) {
        const bx = i * 160 - parallax;
        const peakH = 30 + (((i * 53) % 40) + 40) % 40;
        g.beginPath();
        g.moveTo(bx, HORIZON_Y);
        g.lineTo(bx + 90, HORIZON_Y - peakH);
        g.lineTo(bx + 180, HORIZON_Y);
        g.closePath();
        g.fill();
      }
      g.fillStyle = "#c99a55";
    ...
    function drawTrack() {
      ...
      const nearL = project(-TRACK_HALF_WIDTH - 80, nearZ, 0);
      const nearR = project(TRACK_HALF_WIDTH + 80, nearZ, 0);
      const farL = project(-TRACK_HALF_WIDTH - 80, farZ, 0);
      const farR = project(TRACK_HALF_WIDTH + 80, farZ, 0);
      g.fillStyle = "#dbb066";
      g.beginPath();
      g.moveTo(nearL.x, nearL.y); g.lineTo(farL.x, farL.y); g.lineTo(farR.x, farR.y); g.lineTo(nearR.x, nearR.y);
      g.closePath(); g.fill();

      const trackNearL = project(-TRACK_HALF_WIDTH, nearZ, 0);
      const trackNearR = project(TRACK_HALF_WIDTH, nearZ, 0);
      const trackFarL = project(-TRACK_HALF_WIDTH, farZ, 0);
      const trackFarR = project(TRACK_HALF_WIDTH, farZ, 0);
      g.fillStyle = "#c69a5c";
      g.beginPath();
      g.moveTo(trackNearL.x, trackNearL.y); g.lineTo(trackFarL.x, trackFarL.y); g.lineTo(trackFarR.x, trackFarR.y); g.lineTo(trackNearR.x, trackNearR.y);
      g.closePath(); g.fill();

      g.strokeStyle = "rgba(120,70,30,0.5)";
      g.lineWidth = 3;
      g.beginPath(); g.moveTo(trackNearL.x, trackNearL.y); g.lineTo(trackFarL.x, trackFarL.y); g.stroke();
      g.beginPath(); g.moveTo(trackNearR.x, trackNearR.y); g.lineTo(trackFarR.x, trackFarR.y); g.stroke();

      if (COURSE_LENGTH - player.worldPos < FAR_DRAW_DISTANCE) {
        const fL = project(-TRACK_HALF_WIDTH, COURSE_LENGTH, 60);
        const fR = project(TRACK_HALF_WIDTH, COURSE_LENGTH, 60);
        const fLg = project(-TRACK_HALF_WIDTH, COURSE_LENGTH, 0);
        const fRg = project(TRACK_HALF_WIDTH, COURSE_LENGTH, 0);
    ...
    function drawBuggy() {
      ...
      if (player.airTime > 0) {
        ...
        g.fillStyle = "#5a4020";
        g.beginPath();
        g.ellipse(p.x, p.y, 20 * p.scale * (1 - Math.sin(t * Math.PI) * 0.4), 6 * p.scale, 0, 0, Math.PI * 2);
        g.fill();
        g.restore();
      }
      if (blink) return;
    ...
    function drawHud() {
      ...
      g.fillStyle = "rgba(40,25,10,0.75)";
      ...
      g.fillStyle = "rgba(40,25,10,0.85)";
      g.fillText(`Stalls: ${stalls}`, SCREEN_W - 16, 30);
      g.fillText(`Checkpoints: ${cleanGates}/${gates.filter((gt) => gt.resolved).length || 0}`, SCREEN_W - 16, 50);
    ...
    function startGame() {
      ...
      running = true;
      over = false;
      ctx.setStatus("Go! Thread the checkpoints and reach the finish.");
    }
